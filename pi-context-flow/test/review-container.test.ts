...
for (const scenario of ["security", "timeout", "overflow", "cancel"] as const) {
  test(`real container acceptance: ${scenario}`, { skip: !enabled && "opt-in: CONTEXT_FLOW_CONTAINER_ACCEPTANCE=1", timeout: 90000 }, async (t) => {
    ...
    const wrapper = `#!/bin/sh\nif [ "$1" = run ]; then\n shift\n exec ${quote(cli)} run --label ${quote(label)} "$@"\nfi\nexec ${quote(cli)} "$@"\n`;
    for (const runtime of ["docker", "podman"]) await writeFile(join(bin, runtime), wrapper, { mode: 0o755 });
    await writeFile(join(root, "selected.txt"), "synthetic-selected-input", { mode: 0o600 });
    await mkdir(join(root, "a"));
    await writeFile(join(root, "a", "b.txt"), "nested");
    await writeFile(join(root, "a_b.txt"), "flat");
...
assert open('/inputs/workspace/a/b.txt').read() == 'nested'
assert open('/inputs/workspace/a_b.txt').read() == 'flat'
...
time.sleep(2)` : (scenario === "timeout" || scenario === "cancel") ? "import time; time.sleep(120)" : "import sys,time\ntime.sleep(2)\nwhile True: sys.stdout.write('x'*65536); sys.stdout.flush()";
      const started = Date.now();
      const pending = worker.call({ op: "repl", input: { language: "python", code, evidenceIds: [`evidence://${evidenceId}`], workspacePaths: ["selected.txt", "a/b.txt", "a_b.txt"] } }, 40000).then((value) => ({ value }), (error: unknown) => ({ error }));
      // Observe the daemon while the invocation is live; mocks cannot establish isolation.
      let inspect: any;
      for (let attempt = 0; attempt < 30; attempt++) {
        try { inspect = JSON.parse(command("inspect", owned()[0] ?? name))[0]; break; } catch { await new Promise((r) => setTimeout(r, 100)); }
      }
      if (scenario === "cancel") worker.cancel();
      const settled = await pending;
      if ("error" in settled) throw settled.error;
      const result = settled.value;
      assert.equal(result.available, true, result.reason);
      assert.ok(inspect, `test-owned container must be observed on daemon: ${JSON.stringify(result)}`);
